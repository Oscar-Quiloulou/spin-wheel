class AdManager {
  ...
  selectAd(nextAd) {
    ...
    if (path != null && this.useAbsoluteLinks) {
      if (!this.currentAd.url.includes("https://")) {
        this.currentAd.url =
          "https://unblocked-games.s3.amazonaws.com" + this.currentAd.url;
      }
    }
  }
  ...
  refetchAd(onAdEnd) {
    try {
      ...
      const iframeSrc =
        this.promoLocation +
        `promo.html?name=${encodeURIComponent(
          this.currentAd.name
        )}&img=${encodeURIComponent(
          this.currentAd.image
        )}&url=${encodeURIComponent(this.currentAd.url)}`;
...
window.syncPromoManager = function (fast) {
  ...
  setTimeout(() => {
    ...
    if (typeof window.preroll === "undefined") {
      ...
      if (
        typeof window[window.preroll.config.loaderObjectName] != "undefined"
      ) {
...
window.updateNextUrl = async function (fullUrl) {
  ...
  const destinations = ["d23tskfudfp1j.cloudfront.net"];
